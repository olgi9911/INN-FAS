...
# pila_cpp = cpp_extension.load(name='pila_cpp', sources=['/home/qxl/ddpm/UniINN/flow_net/layers/base/pila.cpp', '/home/qxl/ddpm/UniINN/flow_net/layers/base/pila.cu'],
...
class CPila(nn.Module):
    ...
    def build_jvp_net(self, x):
        class CPilaJVP(nn.Module):
            ...
            def forward(self, x):
                return torch.cat((x,x), dim=2) * self.grad
        ...
        with torch.no_grad():
            ...
            grad = torch.cat((pila_cpp.forward(x-0.2, kabcdmn), -pila_cpp.forward(-x-0.2, kabcdmn)), dim=2)
            grad.div_(1.06)
