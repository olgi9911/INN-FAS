...
class ActNorm(nn.Module):
    ...
    def forward(self, x, logpx=None):
        ...
        bias = self.bias.view([1, -1]).expand_as(x)
        weight = self.weight.view([1, -1]).expand_as(x)
    ...
    def inverse(self, y, logpy=None):
        ...
        bias = self.bias.view([1,  -1]).expand_as(y)
        weight = self.weight.view([1,  -1]).expand_as(y)
    ...
    def _logdetgrad(self, x):
        # return self.weight.view([1, -1]).expand(*x.size()).contiguous().view(x.size(0), -1).sum(1, keepdim=True)
        return self.weight.view([1, -1]).expand(*x.size()).contiguous().sum(-1)
