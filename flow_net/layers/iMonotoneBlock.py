...
from .solvers import RootFind, MonotoneBlockBackward
import copy
import logging
...
class iMonotoneBlock(nn.Module):
    ...
    def forward(self, x, logpx=None):
        ...
        w_value = RootFind.apply(lambda z: nnet_copy(z), math.sqrt(2) * x0).detach()
        ...
        w = MonotoneBlockBackward.apply(lambda z: nnet_copy(z), w_proxy, math.sqrt(2) * x)
        y = math.sqrt(2) * w - x
    ...
    def inverse(self, y, logpy=None):
        ...
        w_value = RootFind.apply(lambda z: -nnet_copy(z), math.sqrt(2) * y0).detach()
        ...
        w = MonotoneBlockBackward.apply(lambda z: -nnet_copy(z), w_proxy, math.sqrt(2) * y)
        x = math.sqrt(2) * w - y

        if logpy is None:
            return x
        else:
            return x, logpy - self._logdetgrad_monotone_resolvent(w).sum(dim=-1)
